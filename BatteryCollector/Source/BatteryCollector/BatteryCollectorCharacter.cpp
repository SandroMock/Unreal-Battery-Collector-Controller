...
void ABatteryCollectorCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	InputComponent->BindAction("Collect", IE_Pressed, this, &ABatteryCollectorCharacter::Collect);
...
void ABatteryCollectorCharacter::Collect()
{
	TArray<AActor*> CollectActor;
	CollectionSphere->GetOverlappingActors(CollectActor);

	float CollectedPower = 0;

	for (int32 iCollect = 0; iCollect < CollectActor.Num(); ++iCollect)
	{
		APickup* const TestPickup = Cast<APickup>(CollectActor[iCollect]);
